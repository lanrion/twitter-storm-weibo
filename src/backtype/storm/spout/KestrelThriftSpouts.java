 ...
 */
public class KestrelThriftSpouts extends BaseRichSpout {
    public static Logger LOG = Logger.getLogger(KestrelThriftSpouts.class);
    ...
    public KestrelThriftSpouts(List<String> hosts, int port, String queueName, Scheme scheme) {
        if(hosts.isEmpty()) {
    ...
    public KestrelThriftSpouts(String hostname, int port, String queueName, Scheme scheme) {
        this(Arrays.asList(hostname), port, queueName, scheme);
    }

    public KestrelThriftSpouts(String hostname, int port, String queueName) {
        this(hostname, port, queueName, new RawScheme());
    }
    
    public KestrelThriftSpouts(List<String> hosts, int port, String queueName) {
...
//                     " queueName=" + _queueName + ", index=" + index + ", host=" + info.host + ")");
	...
	public void ack(Object msgId) {
        ...
        //back on the queue
        try {
        	LOG.info("KestrelThriftSpout acking....");
            if(info.client!=null) {
                HashSet xids = new HashSet();
                xids.add(sourceId.id);
                int info_num = info.client.confirm(_queueName, xids);
                LOG.info("return confirm num: " + info_num);   
            }
        } catch(TException e) {
            blacklist(info, e);            
        }
	...
	public void fail(Object msgId) {
        ...
        try {
        	LOG.info("KestrelThriftSpout failing....");
            if(info.client!=null) {
                HashSet xids = new HashSet();
                xids.add(sourceId.id);
               int info_num =  info.client.abort(_queueName, xids);
             LOG.info("return abort num: " + info_num);
            }
        } catch(TException e) {
            blacklist(info, e);            
        }
