		...
		public void execute(Tuple tuple) {
			...
			try {
				...
				if (!cursor.hasNext()) {
					...
					logger.info("tweet_feed_deleted!!");
					return;
				}
...
//				_collector.emitDirect(2, tuple, new Values(tweet_id));
				logger.info("begin acking......");
				_collector.ack(tuple);
				logger.info("tweet_feed_done!!");

			} catch (Exception e1) {
				logger.info("execue errors："+ e1);
				_collector.emit(tuple, new Values(tweet_id));
	...
	public static void main(String[] args) {
		try {
			...
			KestrelThriftSpout spout = new KestrelThriftSpout(Configs.getKestrels(), 2229, "tweet_id", new StringScheme());
			builder.setSpout("tweet", spout, 10);
			builder.setBolt("feed", new SaveFeeds(), 15).shuffleGrouping("tweet");
